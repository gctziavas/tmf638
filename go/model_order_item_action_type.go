...
const (
	ADD OrderItemActionType = "add"
	MODIFY OrderItemActionType = "modify"
	DELETE OrderItemActionType = "delete"
	NO_CHANGE OrderItemActionType = "noChange"
)

// AssertOrderItemActionTypeRequired checks if the required fields are not zero-ed
func AssertOrderItemActionTypeRequired(obj OrderItemActionType) error {
	return nil
}

// AssertRecurseOrderItemActionTypeRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of OrderItemActionType (e.g. [][]OrderItemActionType), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseOrderItemActionTypeRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aOrderItemActionType, ok := obj.(OrderItemActionType)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertOrderItemActionTypeRequired(aOrderItemActionType)
	})
}
