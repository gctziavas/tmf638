...
func AssertCharacteristicRequired(obj Characteristic) error {
	...
	for _, el := range obj.CharacteristicRelationship {
		if err := AssertCharacteristicRelationshipRequired(el); err != nil {
			return err
		}
	}
	return nil
...
// Accepts only nested slice of Characteristic (e.g. [][]Characteristic), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseCharacteristicRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aCharacteristic, ok := obj.(Characteristic)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertCharacteristicRequired(aCharacteristic)
	})
}
